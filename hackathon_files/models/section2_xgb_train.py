# models/section2_xgb_train.py
# -------------------------------------------------------
# Robust XGBoost trainer (CPU). Tries early stopping; if
# unsupported (older xgboost fit signature), falls back
# to a conservative no-ES config automatically.
# -------------------------------------------------------
...
# 4) Train: try early stopping; if unsupported, fallback
supports_es = True
try:
    model.fit(
        X_train, y_train,
    ...
    # More conservative config to reduce overfitting risk
    model = XGBClassifier(
        n_estimators=400,
        max_depth=5,
        learning_rate=0.06,
...
# 5) Validation + threshold tuning (recall>=0.85 target)
